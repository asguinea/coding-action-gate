...
class BatchT2OptionACalibrationRuleTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.claim = load_script("batchT2_claim", "check_batchT2_claim_boundaries.py")
    ...
    def test_files_state_no_formal_conformal_guarantee_claimed_yet(self):
        for path in THEORY.glob("batch_T2_*.md"):
            text = path.read_text().lower()
            self.assertIn("no formal conformal guarantee claimed yet", text, str(path))
