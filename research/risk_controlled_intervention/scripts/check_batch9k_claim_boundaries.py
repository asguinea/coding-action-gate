#!/usr/bin/env python3
"""Check Batch 9K reports for claim-boundary violations."""
...
REPORT_JSON = REPORTS_DIR / "batch_9k_claim_boundary_check.json"
REPORT_MD = REPORTS_DIR / "batch_9k_claim_boundary_check.md"

REPORTS = [
    "batch_9k_stability_diagnostic_dataset.md",
    "batch_9k_stability_gate_evaluation.md",
    "batch_9k_stability_aware_dual_unit_intervention.md",
    "batch_9k_stability_diagnostic_importance.md",
    "batch_9k_research_assessment.md",
]
...
def check_text(text: str) -> tuple[list[str], list[str]]:
    ...
    for phrase in CONDITIONAL:
        if phrase in lower and f"no formal {phrase} claimed" not in lower and f"not a {phrase}" not in lower and f"not formal {phrase}" not in lower and f"rather than formal {phrase}" not in lower:
            hits.append(phrase)
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "Required safe concepts include benchmark-level, CodeTraceBench-derived, offline proxy, trajectory-level burden, row-level risk, dual-unit, calibration support, stability-aware, fail-closed, domain shift, not production validation, not causal prevention, and no formal conformal guarantee claimed.",
    ]
