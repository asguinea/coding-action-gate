...
describe("workspace boundary helpers", () => {
  it("target equal to root counts as inside", () => {
    const root = path.resolve("/tmp/coding-action-gate-boundary/app");
  ...
  it("sibling prefix path does not count as inside", () => {
    ...
    const sibling = path.resolve(
      "/tmp/coding-action-gate-boundary/app2/file.ts"
    );
  ...
  it("finds paths outside all workspace roots", () => {
    ...
    const outside = path.resolve(
      "/tmp/coding-action-gate-boundary/outside/file.ts"
    );
