# CodingActionGate Local Analytics

CodingActionGate local analytics exists to help local preview evaluators understand product behavior in their own workspace. It is local-only and inspectable.
...
```sh
coding-action-gate analytics summary --json
```
...
```sh
coding-action-gate analytics clear
```
...
This deletes only `.coding-action-gate/analytics/events.jsonl`. It does not delete audit logs, deferred actions, observations, validation state, policy, or all of `.coding-action-gate/`.
...
`false` and `off` are also treated as disabled values. Disabling affects recording only; `coding-action-gate analytics summary` and `coding-action-gate analytics clear --yes` can still read or clear existing local analytics state.

Persistent local enable/disable commands are not implemented yet.

During real repository trials, inspect `coding-action-gate analytics summary` before exporting feedback. Follow [real-repo-trial.md](limitations.md) and avoid sharing raw commands, paths, source code, diffs, environment variables, secrets, repo names, or validation logs.
