...
CodingActionGate evaluates a structured proposal before an external caller applies an action. The caller remains responsible for enforcing the result.
...
Runtime state is local to `.coding-action-gate/`: observations, validation records, deferred actions, audit records, and optional local analytics. Hash chaining can reveal audit-log modification relative to a trusted prior hash; it is not externally anchored proof of authenticity.
